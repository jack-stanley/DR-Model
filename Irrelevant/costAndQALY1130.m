...
for i = 1:size(SD_annual_screen_prices,2)
 for j = 1:length(mean_annual_screen_prices);
    for k = 1:npts%repeat number of patients
        cost(j,k) = SD_annual_screen_prices.*randn(5,3)+mean_annual_screen_prices;
        QALY(j,k) = SD_annual_screen_QALY.*randn(5,3)+mean_annual_screen_QALY;
    end
        if j == 1
...
% options_annual_cost_urban = actual_cost(3,:)
% actual_cost = SD_annual_screen_prices.*randn(5,3)+mean_annual_screen_prices
