function costAndQaly = doMarkov5e(npatients,util,utilSD,costsPerProc,costsPerScreen,dmInPopByAge,MortByAge,...
...
MAXSIMSIZE = 10000; % Maximum subjects within a simulation. Below that we use the actual number of subjects
...
tpmR = zeros(simsize,8,8);
tpmL = zeros(simsize,8,8);
...
% LEFT EYE
    ...
    % Next lines are where the progression in the Markov chain occurs
            ...
            trueStageR(pt) = randsample8(trueStageR(pt),tpmR(pt,trueStageR(pt),:)); % MARKOV!
            prevStageL = trueStageL(pt); % Needed for figuring out when patient went blind
            trueStageL(pt) = randsample8(trueStageL(pt),tpmL(pt,trueStageL(pt),:)); % MARKOV!
