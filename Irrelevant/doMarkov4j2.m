...
SIMSIZE = 100000; % How many subjects within a simulation
...
%             examStageR(pt) = randsample2(NSTAGES,1,true,screenAcc(trueStageR(pt),:)); % Apparent vs true stage of disease X
            examStageR(pt) = randsample8(trueStageR(pt),screenAcc(trueStageR(pt),:)); % Apparent vs true stage of disease X
            ...
            trueStageR(pt) = randsample8(trueStageR(pt),temp1(trueStageR(pt),:));
            if trueStageR(pt) == STAGE_BLIND && prevStageR(pt) ~= STAGE_BLIND %This year patient became blind
                ageWhenBlind(pt) = year;
            end
            % LEFT EYE
            examStageL(pt) = randsample8(trueStageL(pt),screenAcc(trueStageL(pt),:)); % Apparent vs true stage of disease X
            ...
            trueStageL(pt) = randsample8(trueStageL(pt),temp1(trueStageL(pt),:));
