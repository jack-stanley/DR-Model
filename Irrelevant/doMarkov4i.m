function costAndQaly = doMarkov4i(util,utilSD,costsPerProc,costsPerScreen,dmInPopByAge,MortByAge,screenAcc,initScreenInt,morbidityIndex) 
...
SIMSIZE = 200; % How many subjects within a simulation
...
% RIGHT EYE
    ...
    % Next lines are where the progression in the Markov chain occurs
            ...
            trueStageR(pt) = randsample(NSTAGES,1,true,temp1(trueStageR(pt),:));
            if trueStageR(pt) == STAGE_BLIND && prevStageR(pt) ~= STAGE_BLIND %This year patient became blind
               ageWhenBlind(pt) = year;
            end
% LEFT EYE
            examStageL(pt) = randsample(NSTAGES,1,true,screenAcc(trueStageL(pt),:)); % Apparent vs true stage of disease X
    ...
    % Next lines are where the progression in the Markov chain occurs
            ...
            trueStageL(pt) = randsample(NSTAGES,1,true,temp1(trueStageL(pt),:));
...
% Above is performed only if patient is screened
        ...
        ptUtil_over_time(pt,year) = 4; %ptUtils(pt,trueBestEye);
