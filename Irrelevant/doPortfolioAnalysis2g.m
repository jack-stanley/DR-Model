...
for i = 1:nstrategies % Note that we never read the last sheet, which allows us to use it for producing the other sheets
    ...
    for j = 1:NREPEATS
        costQaly = screenSystemV2g(nregions,txt(2:nregions+1,1),num(:,1),num(:,2),num(:,3),num(:,4),num(:,5),...
...
for basestrat = 1:nstrategies
    for i = basestrat:nstrategies % Compare each strategy to the others
        ...
        fprintf('%.0f ',icer);
        fprintf('\n');
        fprintf('ICER Strategy %d vs %d: ICER: %.0f ± %.0f  Unilat blind: %.1f ± %.1f Bilat blind: %.1f ± %.1f\n',...
            i,basestrat,mean(icer),std(icer),mean(changeUniBlind),std(changeUniBlind),mean(changeBlind),std(changeBlind));
    end
end
