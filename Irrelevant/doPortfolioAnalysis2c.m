...
filename = 'Capstone excel V2a.xlsx'
[~,sheets] = xlsfinfo(filename);
nstrategies = size(sheets,2); %Assume strategy 1 is the base strategy


%% Define ICER calculation parameters
NREPEATS = 4; % How many times we repeat the calculations to ensure low variability
...
for i = 1:nstrategies
    ...
    for j = 1:NREPEATS
        costQaly = screenSystemV2c(nregions,txt(2:nregions+1,1),num(:,1),num(:,2),num(:,3),num(:,4),num(:,5),num(:,6),num(:,7),num(:,8),num(:,9),num(:,10),num(:,11));
