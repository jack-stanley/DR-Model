...
NREPEATS = 10000;       % How many times we repeat the calculations to ensure low variability
...
for i = 1:nstrategies % Note that we never read the last sheet, which allows us to use it for producing the other sheets
    ...
    for j = 1:NREPEATS
        costQaly = screenSystemV2j(nregions,txt(2:nregions+1,1),num(:,1),num(:,2),num(:,3),num(:,4),num(:,5),...
