function costAndQaly = doMarkov5d(npatients,util,utilSD,costsPerProc,costsPerScreen,dmInPopByAge,MortByAge,...
...
SIMYEARS = 30; %Length of the simulation. NOTE WE DON'T ADD NEW PATIENTS AS THEY AGE OR TURN 18
...
%            break
        ...
        if (year >= lastScreened + screeningInt) && (rand() < complianceRate) % if they are due for a screening and likely to show up
...
% All of the following is done if patient is screened
                ...
                examStageR = randsample8(trueStageR,screenAcc(trueStageR,:)); % Apparent vs true stage of disease R eye
                examStageL = randsample8(trueStageL,screenAcc(trueStageL,:)); % Apparent vs true stage of disease L eye
                canSeeOphth = true;
                if (screenRefer(examStageR) == 1) || (screenRefer(examStageL) == 1) % One or both eyes requires referral to an ophthalmologist
                    % Note that ophthalmologists don't refer to ophthalmologists
                	resources(year,RESOURCESOPHTH) = resources(year,RESOURCEOPHTH) - 1; % Use up a ophth referral resource -- 1 per screen
...
fprintf('Resource utilization %.1f%%\n',(SIMYEARS * resourcesPerYear - sum(resources,1)) * 100 / (SIMYEARS * resourcesPerYear));
